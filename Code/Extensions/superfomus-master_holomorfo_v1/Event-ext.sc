...
+ Event {
	...
	asFomusString {
		...
		this.keys.includes(\instru).if({
			//			"Includes instru".postln;
			outString = "part " ++ this[\instru].asString
			//outString = "part " ++ "instru";
		});
		...
		this.keys.includes(\voice).if({
			outString = outString ++" voice " ++ this[\voice].asString
		});

		this.keys.includes(\mark).if({
			outString = outString ++"" ++ this[\mark].asString
		});
